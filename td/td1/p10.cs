...
HighpassRC filter7000 = HighpassRC.CreateForCutoffFrequency(7000, 470e-9);
...
public abstract class RCFilter
{
    protected Resistance R;
    protected Capacitor C;

    public RCFilter(double R_ohms, double C_farads)
    {
...
public class LowpassRC : RCFilter
{
    public LowpassRC(double R_ohms, double C_farads) : base(R_ohms, C_farads)
    {
    }
...
public class HighpassRC : RCFilter
{
    public HighpassRC(double R_ohms, double C_farads) : base(R_ohms, C_farads)
    {
    }

    public static HighpassRC CreateForCutoffFrequency(double fHz, double cF)
    {
        double R_ohms = 1 / (2 * Math.PI * fHz * cF);
        return new HighpassRC(R_ohms, cF);
    }
    ...
    public override string ToString()
    {
        return $"HighpassRC(R={R.GetResistance()} Ω, C={C.GetCapacitance()} F, f0={GetCutoffFrequency()} Hz)";
    }
}